...
def build_fireflies(led, opts):
	colors = literal_eval(opts.get('--colors')) if opts.get('--colors') else [randcolor() for i in range(random.randint(2,6))]
	width = int(opts.get('--width')) if opts.get('--width') else random.randint(1, int(STRIP_LENGTH/len(colors)))
	count = int(opts.get('--count')) if opts.get('--count') else random.randint(1, int(STRIP_LENGTH/len(colors)))
