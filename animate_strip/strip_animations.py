...
class Breathe(BaseStripAnim):
    def randcolor(self):
	print random.randint(0,255)
	return (random.randint(0,255),random.randint(0,255),random.randint(0,255))
	
    def __init__(self, led, cycles, start=0, end=-1):
        super(Breathe, self).__init__(led, start, end)
    ...
    def step(self, amt = 1):
	    ...
	    if y < 0.0 and abs(y + 1.0) < 0.005:
		self._color = self.randcolor()

            r, g, b = self._color
            c2 = (int((r + y*r)/2), int((g + y*g)/2), int((b + y*b)/2))
            self._led.fill(c2)
	    self._step += amt
